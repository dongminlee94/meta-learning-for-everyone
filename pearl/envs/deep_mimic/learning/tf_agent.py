...
class TFAgent(RLAgent):
  ...
  def _get_output_path(self):
    ...
    file_path = self.output_dir + '/agents' + str(self.id) + '_model.ckpt'
    return file_path

  def _get_int_output_path(self):
    assert (self.int_output_dir != '')
    file_path = self.int_output_dir + (
        '/agents{:d}_models/agents{:d}_int_model_{:010d}.ckpt').format(self.id, self.id, self.iter)
    return file_path
